...
from .functions.prompt_constructor import (
    construct_sys_prompt_for_plan,
    construct_sys_prompt_for_hotels,
    construct_sys_prompt_for_improvement,
    construct_sys_prompt_for_hotel_improvement,
)
...
def improve(request):
    try:
        ...
        if len(check) < 200:
            ...
            sys_prompt_2 = construct_sys_prompt_for_hotel_improvement(
                hotels, original_plan
            )
            print(sys_prompt_2)
            output = send_prompt(
                client, sys_prompt_2, user_input, t=0.5, max_tokens=1500
            )
            return JsonResponse({"data": output})
        else:
            output = check
